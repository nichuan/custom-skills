...
只有完整实现新需求时，编码前才执行设计门禁：至少确认输入与输出字段来源，涉及数据库关联、外部服务、状态分支或多产物协同时再补齐对应契约。先阅读 [references/delivery-gates.md](references/delivery-gates.md)，并在需求目录生成 `design-gates.md`；未知事实必须标记为 `TBD`。
