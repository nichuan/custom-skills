...
只要目标脚本已经存在非模板的历史实现，增量更新默认遵循**最小改动原则**，该原则同时适用于本地已有脚本和从平台拉取的历史脚本：

- 只修改本次需求必需的逻辑；保留原有入口、整体结构、变量命名、代码风格、格式、注释、日志习惯、分支顺序和历史兼容行为。
- 新逻辑尽量贴近对应的现有分支或字段，以局部追加、局部替换为主；不要顺手格式化、重命名、抽取公共方法、调整目录、统一风格、替换 API/写法或修复与本次需求无关的问题。
- 即使历史写法不够理想，只要不阻止本次需求正确实现，就不要改写；只有为兼容新逻辑确实必须触碰历史代码时，才做范围最小的配套调整，并在交付说明中说明原因。
- 用户明确提出“优化整体代码结构/逻辑”“重构代码逻辑”“重写代码逻辑”等整体性指令后，才允许扩大改动范围；除此之外，不把代码质量优化或风格统一当作隐含需求。
- 修改完成后检查 diff，确认没有与本次需求无关的历史逻辑或风格变化；交付时说明新增/调整的局部逻辑以及明确保留的历史行为。
...
2. 按“历史脚本增量更新原则”做最小范围修改，仅加入或调整本次需求所需逻辑，保留原入口、结构、格式、注释和代码风格。
3. JavaScript 运行 `node --check <文件>`；若已有与改动直接相关且成本很低的检查，再运行该项。
4. 检查 diff 无无关改动，报告修改文件、字段取值变化、保留的历史逻辑和检查结果。
...
4. 确认后需求根目录固定为 `<delivery-root>/<issue>/<tenant>/`。先检查路径是否为目录或可创建、最近已存在父目录是否可写；若不在当前 Agent 可写工作区内，说明目标路径并请求相应文件系统授权。
...
- 只有用户明确要求“保存、发布、部署或更新到 DEV”时，才调用 `independent_script_save` 或 `adapter_deploy` 生成写计划；必须携带最近一次 `get` 的预期版本。展示计划并结束当前轮，收到用户后续明确确认后，才用相同参数和一次性 `confirmation_token` 执行。发现版本冲突就重新读取并报告差异，不能覆盖并发修改。
