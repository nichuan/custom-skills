...
本 Skill 是专门为 SRM 盘古（订单履约域）设计的 SQL 生成助手，覆盖 **采购订单（SODR）→ 发货（SLOD/老送货单）→ 收货事务（SINV_RCV）→ 导出（结算/外部/商城/委外）** 全链路。采用与 `ssrc-sql-generator` 相同的分层设计：**「本地仅沉淀业务语义、结构事实走 MCP」**，表结构通过 **zhenyun-pangu-mcp 的 Archery 工具** 实时获取，SQL 模板通过 **zhenyun-pangu-mcp 认知层（Supabase）** 按盘古专属分类检索与沉淀。
...
> - 模板库通过 **category/system/title 前缀/keywords** 四重区分，避免检索串扰（详见 SKILL.md「盘古模板约定」）。
...
1. `search_sql_templates` 按盘古分类检索模板复用
...
- **zhenyun-pangu-mcp 认知层**：Supabase 模板库（与 ssrc 共库，用分类区分）
