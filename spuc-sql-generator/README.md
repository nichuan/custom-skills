...
本 Skill 是专门为 SRM 盘古（订单履约域）设计的 SQL 生成助手，覆盖 **采购订单（SODR）→ 发货（SLOD/老送货单）→ 收货事务（SINV_RCV）→ 导出（结算/外部/商城/委外）** 全链路。采用与 `ssrc-sql-generator` 相同的分层设计：**「本地仅沉淀业务语义、结构事实走 MCP」**，表结构通过 **sql-ops MCP** 实时获取，SQL 模板通过 **sql-template MCP（Supabase）** 按盘古专属分类检索与沉淀。
...
- **sql-ops MCP**：生产库 SAAS-SRM-PROD（`srm` / `srm_logistics_delivery`）
