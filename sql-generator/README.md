...
> **重要**: 本 Skill 同时覆盖询价单(RFX开头)和新招标单(BID开头)两种寻源场景。两者共用同一套 `ssrc_rfx_*` 数据库表。区分字段是 `ssrc_rfx_header.secondary_source_category`（新招标单 = `'NEW_BID'`），评标/结果等关联表中两者**共用同一 `source_from = 'RFX'` 上下文**（新招标不再使用 `'BID'`，`'BID'` 仅指几乎不用的老招标），仅前端术语叫法不同（如"待定标"="待核价"、"评标"="评分"等）。
...
- 生成的 SQL 必须包含关键注释说明
- ⚠️ 新招标单(BID)查询时与询价单完全共用：评标/结果表 `source_from` 仍为 `'RFX'`（不要写成 `'BID'`），单据类型区分用 `ssrc_rfx_header.secondary_source_category = 'NEW_BID'`
