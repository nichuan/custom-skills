...
- **UPDATE/DELETE 安全规则（必须严格遵守）**:
  ...
  - WHERE 条件必须使用 `tenant_id` + **主键**（如 `rfx_header_id`），禁止使用业务单号（如 `rfx_num`）作为更新/删除条件
  - 当用户提供的是业务单号而非主键时，必须分两步执行：
    1. **查询**: 通过业务单号查出主键ID
    2. **修改**: 用查到的主键ID 构造 UPDATE/DELETE 语句
  - 示例：
    ```sql
    -- 步骤1: 通过单号查主键
    SELECT rfx_header_id FROM ssrc_rfx_header WHERE tenant_id = ? AND rfx_num = 'RFX2026022400003';
    -- 步骤2: 用主键更新（正确）
    UPDATE ssrc_rfx_header SET rfx_status = 'CHECK_PENDING' WHERE tenant_id = ? AND rfx_header_id = ?;
    ```
- **询价单状态同步规则（必须严格遵守）**:
  ...
  - 修改询价单状态时，必须同时更新两张表，缺一不可：
    ...
    -- 同步更新扩展表状态
    UPDATE ssrc_rfx_header_expand SET rfx_real_status = '{new_status}' WHERE tenant_id = ? AND rfx_header_id = ?;
    ```
