...
description: Java 微服务故障排查助手。仅在用户描述了异常、报错、traceId、日志、接口失败、超时、线上故障，或明确询问操作/配置/升级导致的问题时使用；通过 log-ops MCP 分析日志/调用链，并通过 gitlab-code MCP 定位源码，按需通过 sql-ops MCP 查询数据。仅提到 SRM 业务模块、查询数据、生成 SQL 或数据修复时不要触发。
---
