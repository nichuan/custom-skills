# 适配器 JS 脚本（埋点二开方式，存库非 Git）

> 企业事实层。标准代码内置「适配器（Adapter）」处理，核心是执行一段 **JS 脚本**扩展/覆盖标准逻辑。脚本内容存在数据库（不放在仓库），以 **Base64** 编码存储。适配器脚本内的数据库事务与标准逻辑是同一事务。
>
> ⚠️ **本篇只覆盖适配器埋点脚本**。独立脚本（Marmot 脚本库，定时任务/打印模板/导入等非挂钩点执行）是另一套体系，存于 rel-table 宽表，见 `standalone-script.md`。按执行链、task_code 和场景信号选择目标体系；第一候选未命中或信号冲突时再查另一套。
