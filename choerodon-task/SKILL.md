...
统一使用 `zhenyun-pangun-mcp` 的 `choerodon_*` 系列工具。**所有参数必须取真实值，严禁瞎猜**（任务号/经办人必须先查真实 id 再传）。
...
- **写评论例外（有明确工具）**：zhenyun-pangun-mcp 提供 `choerodon_add_comment`（写接口，有副作用）。**仅当用户明确要求"把内容写到猪齿鱼评论区"时才可使用**，且必须先展示内容向用户确认、确认后再写入。日常查询/排查流程中**不得**自动调用它。
- **评论格式**：用 `choerodon_add_comment` 写评论时，内容应整理为**标准 Markdown**（标题/列表/代码块/加粗），由工具自动转 HTML 在评论区美观展示；不得写入纯堆砌文本。
