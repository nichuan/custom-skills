---
name: choerodon-task
description: 猪齿鱼（Choerodon）跨项目协作任务只读查询助手。用于先按项目 ID、名称或编码定位当前账号可访问的项目，再按任务号、经办人、关键词或状态查询 issue，并查看评论、状态流转和附件。用户要求开发 Marmot 纯二开需求时不要使用本 Skill，应使用 srm-requirement-delivery；本 Skill 不改写猪齿鱼数据或业务库。
---
